...
/**
 The `LKLocationManager` protocol defines a set of methods that a class must
 implement in order to serve as the location manager of an `LKMapView`. A location
 ...
 To receive location updates from an object that conforms to the `LKLocationManager`
 protocol, use the optional methods available in the `LKLocationManagerDelegate` protocol.
 */
@protocol LKLocationManager <NSObject>
...
/**
 ...
 The default value of this property is `kCLDistanceFilterNone` when `LKMapView` uses its
...
/**
 ...
 The default value is `kCLLocationAccuracyBest` when `LKMapView` uses its
 default location manager.
...
/**
 ...
 The default value is `CLActivityTypeOther` when `LKMapView` uses its
 default location manager.
...
/**
 ...
 Do not set the location manager’s delegate yourself. `LKMapView` sets this property
 after the location manager becomes `LKMapView`’s location manager.
 */
@property (nonatomic, weak) id<LKLocationManagerDelegate> delegate;
...
/**
 The `LKLocationManagerDelegate` protocol defines a set of methods that respond
 to location updates from an `LKLocationManager` object that is serving as the
 location manager of an `LKMapView`.
 */
...
/**
 ...
 contains multiple `CLLocation` objects when `LKMapView` uses  its
 default location manager.
 */
- (void)locationManager:(id<LKLocationManager>)manager
 ...
 */
- (BOOL)locationManagerShouldDisplayHeadingCalibration:(id<LKLocationManager>)manager;
