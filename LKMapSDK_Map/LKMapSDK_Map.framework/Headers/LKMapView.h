 ...
 *
 * 接收用户位置需要实现以下代理方法：
 * `-[LKMapViewDelegate mapView:didUpdateUserLocation:]` 和
 * `-[LKMapViewDelegate mapView:didFailToLocateUserWithError:]`
 *
 * 如果将该属性设置为 `nil` 或者没有自定义manager，那么将使用默认location manager。
 *
 * @note `LKMapView`使用默认location manager。如果想使用自己定义的location manager，需要确保在设置
 * `showsUserLocation` 为 `YES`之前设置自定义的location manager。 如果想重新使用默认location manager时，只需要设置改属性为`nil`
 */
@property (nonatomic, null_resettable) id<LKLocationManager> locationManager;
